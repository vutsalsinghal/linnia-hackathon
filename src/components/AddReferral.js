...
export class AddReferral extends Component {
	...
	handleSubmit = async (event) => {
		...
		const eth_wallet = this.state.eth_address;
		let result;
	...
	render() {
		return (
			<Form onSubmit={this.handleSubmit} error={!!this.state.errorMessage}>
				...
				<Form.Field>
					<label htmlFor='eth_address'>Ethereum Wallet Public Key</label>
					<input id='eth_address' type='text' value={this.state.eth_address} onChange={event => this.setState({ eth_address: event.target.value })} placeholder='Public Key' />
				</Form.Field>
